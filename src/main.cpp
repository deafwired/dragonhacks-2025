...
void setup()
{
  ...
  // --- Seed the PRNG ---
  randomSeed(analogRead(A0)); // Use an unused analog pin for better randomness
...
void loop()
{
  ...
  // --- Process Read Result ---
  if (bytesRead >= 0) {
    // Read was successful (including reading a valid header indicating 0 length)
    ...
    // ... (rest of the print statements for payload data) ...
    ...
    delay(1000); // Show success message
     ...
     // Optional: Add logic here to decide if you want to write new data
     // even if the read was successful. For now, we just read and release.
     Serial.println("Data read successfully. No automatic write performed.");


  } else if (bytesRead == -2) {
    // Specific error: Invalid header length found
    Serial.println(F("Read Error: Invalid header length found on card!"));
    Serial.println(F("Card data seems corrupted or uninitialized."));
    setLCDMessageCentered("Invalid Header!", 0);
    setLCDMessageCentered("Overwriting...", 1);
    Serial.println(F("Attempting to overwrite with default password..."));
    delay(1500); // Give user time to see the message

    // --- Prepare and Write Default Password ---
    String defaultPassword = "DefaultPassword123"; // Example default password
    byte passwordBytes[MAX_PAYLOAD_SIZE];
    uint16_t passwordLength = defaultPassword.length();

    // Ensure default doesn't exceed max size (shouldn't with this example)
    if (passwordLength > MAX_PAYLOAD_SIZE) {
        passwordLength = MAX_PAYLOAD_SIZE;
    }
    memcpy(passwordBytes, defaultPassword.c_str(), passwordLength);

    if (writeUserDataToNfc(DATA_TYPE_PASSWORD, passwordBytes, passwordLength)) {
        Serial.println(F("Overwrite successful. Card initialized with default password."));
        setLCDMessageCentered("Overwrite OK", 1);
    } else {
        Serial.println(F("Overwrite failed. Card might still be corrupted or write-protected."));
        setLCDMessageCentered("Overwrite FAIL", 1);
        // Note: writeUserDataToNfc already calls StopCrypto1 on failure
    }

  } else {
    // Other read errors (e.g., buffer too small (-1), read fail (-3), auth fail inside read)
    Serial.print(F("Failed to read user data. Error code: ")); Serial.println(bytesRead);
    setLCDMessageCentered("Read Failed!", 1);
    // Note: readUserDataFromNfc already calls StopCrypto1 on failure
  }

  // --- Cleanup for this card ---
  // Halt PICC. StopCrypto1 is called within the read/write functions on failure,
  // or needs to be called here if the operation succeeded.
  // It's safe to call StopCrypto1 even if it was already called.
  mfrc522.PICC_HaltA();
...
bool authenticateBlock(byte blockAddr) {
  ...
  if (status != MFRC522::STATUS_OK) {
    // Keep auth failure message concise for cleaner logs during multi-block ops
...
bool readBlockFromNfc(byte blockAddr, byte buffer[], byte bufferSize) {
  ...
  if (status != MFRC522::STATUS_OK) {
    ...
    // Don't stop crypto here, let the calling function handle cleanup
    return false;
  }
  return true;
}
...
bool writeBlockToNfc(byte blockAddr, byte buffer[], byte bufferSize) {
  ...
  // Crucial safety check: only allow writing to designated user data blocks
...
int readUserDataFromNfc(byte* dataType, uint16_t* dataLength, byte dataBuffer[], int bufferCapacity) {
    ...
    *dataLength = 0;            // Default to zero length
    ...
    if (!readBlockFromNfc(firstUserBlockAddr, firstBlockBuffer, sizeof(firstBlockBuffer))) {
        ...
        mfrc522.PCD_StopCrypto1(); // Stop crypto on failure
        return -3; // Indicate failure to read header block
    }

    // 2. Parse the header from the first block buffer
    *dataType = firstBlockBuffer[0];
    // Combine bytes 1 and 2 into uint16_t (Little Endian)
    *dataLength = (uint16_t)(firstBlockBuffer[2] << 8) | firstBlockBuffer[1];

    // 3. Validate the header data - THIS IS THE CRITICAL CHECK
    if (*dataLength > MAX_PAYLOAD_SIZE) {
        Serial.print(F("Read User Data Error: Header length (")); Serial.print(*dataLength);
        Serial.print(F(") exceeds max payload size (")); Serial.print(MAX_PAYLOAD_SIZE); Serial.println(F("). Invalid header."));
        mfrc522.PCD_StopCrypto1(); // Stop crypto on failure
        // Don't reset dataType/dataLength here, let the caller see the invalid values if needed
        return -2; // Indicate invalid header data (length too big)
    }
    ...
    while (bytesSuccessfullyRead < payloadBytesToRead && currentBlockIndex < NUM_USER_DATA_BLOCKS) {
        ...
        if (!readBlockFromNfc(currentBlockAddr, tempBlockBuffer, sizeof(tempBlockBuffer))) {
            ...
            mfrc522.PCD_StopCrypto1(); // Stop crypto on failure
            return -2; // Indicate read failure during payload read (treat as header/data error)
        }
    ...
    if (bytesSuccessfullyRead != payloadBytesToRead) {
         ...
         mfrc522.PCD_StopCrypto1(); // Stop crypto on failure
         return -2; // Indicate read failure (length mismatch - treat as header/data error)
    }

    // Success! Return the number of payload bytes read.
    // Crypto remains active, caller handles HaltA and StopCrypto1.
...
bool writeUserDataToNfc(byte dataType, byte payloadBuffer[], uint16_t payloadLength) {
    ...
    int blocksNeeded = (totalBytesToWrite + BLOCK_SIZE - 1) / BLOCK_SIZE; // Ceiling division
    ...
    for (int i = 0; i < blocksNeeded; i++) {
        ...
        if (i == 0) {
            ...
            tempBlockBuffer[1] = (byte)(payloadLength & 0xFF);         // Low byte
            tempBlockBuffer[2] = (byte)((payloadLength >> 8) & 0xFF); // High byte
    ...
    if (success) {
        // Serial.print(F("Payload write phase complete. Zeroing remaining ")); // Optional: Less verbose log
    ...
    // Caller handles HaltA and StopCrypto1 on success
    return success;
}


/**
 * @brief Centers text on the LCD.
